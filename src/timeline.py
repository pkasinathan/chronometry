...
def categorize_activity(summary: str) -> Tuple[str, str, str]:
    ...
    # Define category patterns - using Netflix red theme with subtle variations
    # All colors are muted to maintain Netflix's minimal aesthetic
    categories = {
        'code': {
            'keywords': ['coding', 'programming', 'ide', 'cursor', 'vscode', 'terminal', 'git', 'commit', 'debug', 'python', 'java', 'javascript'],
            'icon': '💻',
            'color': '#E50914'  # Netflix red for primary work
        },
        'meeting': {
        ...
        'video': {
            'keywords': ['youtube', 'video', 'watching', 'netflix'],
            'icon': '▶️',
            'color': '#757575'  # Gray
        },
        'social': {
        ...
        'learning': {
            ...
            'icon': '📚',
            'color': '#E50914'  # Netflix red for productive
        },
        'design': {
...
def generate_timeline_html(activities: List[Dict], stats: Dict, date: datetime) -> str:
    ...
    all_categories = sorted(set(a['category'] for a in activities))
