...
class TestDeduplicateBatchAnnotations:
    ...
    def test_batch_annotations_deduplicated(self):
        ...
        annotations = [
            {'datetime': datetime(2025, 11, 1, 10, 0), 'summary': 'Coding task', 'batch_size': 3, 'image_file': 'img1.png'},
            {'datetime': datetime(2025, 11, 1, 10, 15), 'summary': 'Coding task', 'batch_size': 3, 'image_file': 'img2.png'},
            {'datetime': datetime(2025, 11, 1, 10, 30), 'summary': 'Coding task', 'batch_size': 3, 'image_file': 'img3.png'}
        ]
...
class TestCalculateStats:
    ...
    def test_calculate_empty_activities(self):
        ...
        assert stats['distraction_percentage'] == 0
    ...
    def test_category_breakdown(self):
        ...
        activities = [
            {
                'start_time': datetime(2025, 11, 1, 10, 0),
                'end_time': datetime(2025, 11, 1, 10, 20),
                'category': 'Code'
            },
            {
...
class TestGenerateTimelineHTML:
    ...
    def test_generate_html_with_activities(self):
        ...
        assert '💻' in html
        assert 'focus_percentage' in html
...
class TestGenerateTimeline:
    ...
    @patch('src.timeline.load_annotations')
    @patch('src.timeline.group_activities')
    @patch('src.timeline.calculate_stats')
    def test_generate_timeline_success(self, mock_stats, mock_group, mock_load, tmp_path):
        """Test successful timeline generation."""
        # Setup mocks
        mock_load.return_value = [
            {'datetime': datetime(2025, 11, 1, 10, 0), 'summary': 'Test', 'all_frames': []}
        ]
