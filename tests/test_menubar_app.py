...
class TestInitialization:
    ...
    @patch('src.menubar_app.load_config')
    @patch('src.menubar_app.rumps.App.__init__')
    def test_init_loads_config(self, mock_rumps_init, mock_load):
        ...
        from src.menubar_app import ChronometryApp
    ...
    @patch('src.menubar_app.load_config')
    @patch('src.menubar_app.rumps.App.__init__')
    @patch('src.menubar_app.rumps.alert')
...
class TestCaptureControl:
    ...
    def mock_app(self):
        ...
        with patch('src.menubar_app.load_config') as mock_load:
            with patch('src.menubar_app.rumps.App.__init__', return_value=None):
                mock_load.return_value = {
                    'root_dir': '/tmp/test',
                    'capture': {'capture_interval_seconds': 900, 'monitor_index': 1, 
                               'retention_days': 30, 'startup_delay_seconds': 5},
                    'annotation': {'batch_size': 4},
    ...
    @patch('src.menubar_app.show_notification')
...
class TestManualActions:
    ...
    @patch('src.menubar_app.capture_single_frame')
    @patch('src.menubar_app.threading.Thread')
    ...
    @patch('src.menubar_app.capture_region_interactive')
    @patch('src.menubar_app.threading.Thread')
    ...
    @patch('src.menubar_app.annotate_frames')
    @patch('src.menubar_app.show_notification')
    @patch('src.menubar_app.threading.Thread')
    ...
    @patch('src.menubar_app.generate_timeline')
    @patch('src.menubar_app.show_notification')
    @patch('src.menubar_app.threading.Thread')
    ...
    @patch('src.menubar_app.generate_daily_digest')
    @patch('src.menubar_app.show_notification')
    @patch('src.menubar_app.threading.Thread')
    ...
    @patch('src.menubar_app.webbrowser.open')
    ...
    def test_open_timeline(self, mock_path, mock_browser, mock_app):
        ...
        with patch('src.menubar_app.format_date', return_value='2025-11-01'):
...
class TestCaptureLoop:
    ...
    @patch('src.menubar_app.mss.mss')
    @patch('src.menubar_app.capture_iteration')
    @patch('src.menubar_app.time.sleep')
    ...
    @patch('src.menubar_app.mss.mss')
    @patch('src.menubar_app.capture_iteration')
    @patch('src.menubar_app.cleanup_old_data')
    @patch('src.menubar_app.time.sleep')
    @patch('src.menubar_app.time.time')
    def test_capture_loop_periodic_cleanup(self, mock_time, mock_sleep, mock_cleanup, mock_iteration, mock_mss, mock_app):
...
class TestAnnotationLoop:
    ...
    @patch('src.menubar_app.annotate_frames')
    @patch('src.menubar_app.count_unannotated_frames')
    @patch('src.menubar_app.time.sleep')
    @patch('src.menubar_app.time.time')
    ...
    def test_annotation_loop_respects_pause(self, mock_sleep, mock_count, mock_app):
        ...
        with patch('src.menubar_app.annotate_frames') as mock_annotate:
            mock_app._annotation_loop()
...
class TestUIActions:
    ...
    @patch('src.menubar_app.subprocess.run')
    @patch('src.menubar_app.Path')
    ...
    @patch('src.menubar_app.subprocess.run')
    @patch('src.menubar_app.sys.exit')
...
class TestHotkeySetup:
    ...
    @patch('src.menubar_app.keyboard.Listener')
    @patch('src.menubar_app.keyboard.HotKey')
