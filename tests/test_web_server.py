...
from src.web_server import app, init_config
...
class TestConfiguration:
    ...
    @patch('src.web_server.ensure_absolute_path')
    ...
    def test_init_config_uses_defaults(self, mock_load):
        ...
        assert app.config['SECRET_KEY'] == 'chronometry-secret-key-2025'
...
class TestDataEndpoints:
    ...
    def setup_config(self):
        ...
        with patch('src.web_server.config', {
            'root_dir': '/tmp/test',
    ...
    @patch('src.web_server.load_annotations')
    @patch('src.web_server.group_activities')
    @patch('src.web_server.calculate_stats')
    @patch('src.web_server.Path')
    ...
    @patch('src.web_server.load_annotations')
    @patch('src.web_server.group_activities')
    @patch('src.web_server.get_daily_dir')
    def test_get_timeline_by_date(self, mock_daily_dir, mock_group, mock_load, client, setup_config, tmp_path):
        ...
        mock_load.return_value = [
            {'datetime': datetime(2025, 11, 1, 10, 0), 'summary': 'Test'}
        ]
        ...
        with patch('src.web_server.calculate_stats') as mock_stats:
    ...
    @patch('src.web_server.get_or_generate_digest')
    ...
    def test_search_activities(self, mock_daily_dir, mock_group, mock_load, client, setup_config, tmp_path):
        ...
        mock_load.return_value = [
            {'datetime': datetime(2025, 11, 1, 10, 0), 'summary': 'Python coding'}
        ]
        mock_group.return_value = [
            {
                'category': 'Code',
    ...
    @patch('src.web_server.TokenUsageTracker')
    @patch('src.web_server.load_annotations')
    @patch('src.web_server.group_activities')
    @patch('src.web_server.calculate_stats')
    @patch('src.web_server.get_daily_dir')
    def test_get_analytics(self, mock_daily_dir, mock_stats, mock_group, mock_load,
                          mock_tracker_class, client, setup_config, tmp_path):
...
class TestFrameEndpoints:
    ...
    def setup_config(self):
        """Setup configuration for tests."""
        with patch('src.web_server.config', {'root_dir': '/tmp/test'}):
            yield
    
    @patch('src.web_server.load_json')
    @patch('src.web_server.get_daily_dir')
    def test_get_frames(self, mock_daily_dir, mock_load_json, client, setup_config, tmp_path):
        ...
        json_file.write_text(json.dumps({
            ...
            'image_file': '20251101_100000.png'
        }))
    ...
    @patch('src.web_server.send_file')
    @patch('src.web_server.ensure_absolute_path')
    @patch('src.web_server.get_daily_dir')
    ...
    @patch('src.web_server.get_daily_dir')
    def test_get_frame_image_not_found(self, mock_daily_dir, client, setup_config, tmp_path):
...
class TestDatesEndpoint:
    ...
    def test_get_available_dates(self, mock_path_class, client, setup_config, tmp_path):
        ...
        with patch('src.web_server.Path', return_value=mock_path_inst):
...
class TestErrorHandling:
    ...
    def test_invalid_date_format(self, client, setup_config):
        ...
        with patch('src.web_server.parse_date', side_effect=ValueError("Invalid date")):
...
class TestConfigurationUpdate:
    ...
    @patch('src.web_server.init_config')
    @patch('src.web_server.Path')
    def test_update_config_user_config(self, mock_path_class, mock_init, client, tmp_path):
        ...
        with patch('builtins.open', mock_open(read_data='capture:\n  capture_interval_seconds: 900\n')):
            with patch('src.web_server.yaml.safe_load', return_value={'capture': {'capture_interval_seconds': 900}}):
...
class TestWebSocketEvents:
    ...
    def test_handle_connect(self):
        ...
        from src.web_server import handle_connect
        
        with patch('src.web_server.emit') as mock_emit:
            handle_connect()
    ...
    def test_handle_disconnect(self):
        ...
        from src.web_server import handle_disconnect
    ...
    def test_handle_subscribe_live(self):
        ...
        from src.web_server import handle_subscribe_live
    ...
    def test_broadcast_new_frame(self):
        ...
        from src.web_server import broadcast_new_frame
        
        with patch('src.web_server.socketio.emit') as mock_emit:
    ...
    def test_broadcast_new_activity(self):
        ...
        from src.web_server import broadcast_new_activity
